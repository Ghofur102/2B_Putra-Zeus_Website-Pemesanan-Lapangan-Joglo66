...
class ForgotPasswordController extends Controller
{
    ...
    public function reset(Request $request)
    {
        ...
        $emailToken = EmailVerificationToken::where('token', hash('sha256', $validated['token']))
                                            ->where('type', 'reset')
                                            ->first();

        if (!$emailToken) {
            throw ValidationException::withMessages(['token' => 'Token reset password tidak valid']);
        }

        if ($emailToken->isExpired()) {
            $emailToken->delete();
            return redirect()->route('password.request')->with('error', 'Link reset password sudah kadaluarsa (15 menit)');
        }

        if ($emailToken->isUsed()) {
            return redirect()->route('password.request')->with('error', 'Link reset password sudah pernah digunakan');
        }
