...
<div class="max-w-6xl mx-auto pb-8">
    ...
    <div class="bg-amber-50 border border-amber-200 p-4 rounded-tenant-lg mb-6 shadow-tenant-sm border-l-4 border-l-amber-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        ...
        <div id="slot-section" data-view-date="{{ $selectedDate }}" data-formatted-date="{{ \Carbon\Carbon::parse($selectedDate)->translatedFormat('d F Y') }}" class="bg-white p-6 rounded-tenant-lg border border-gray-200 shadow-tenant-sm transition-opacity duration-300 flex flex-col">
            ...
            @if (count($slots) === 0)
                <div class="grid grid-cols-2 gap-4">
                    <div class="col-span-2 text-center py-10 text-gray-400 text-sm bg-gray-50 rounded-tenant-md border border-dashed border-gray-300">
        ...
        <div id="calendar-section" class="bg-white p-6 rounded-tenant-lg border border-gray-200 shadow-tenant-sm h-fit transition-opacity duration-300">
            ...
            <div class="grid grid-cols-7 gap-2 text-center" id="calendarDays">
                @foreach ($calendar as $cell)
                    ...
                    @elseif ($isSelected)
                        <a href="{{ $linkUrl }}" class="ajax-link w-8 h-8 flex items-center justify-center mx-auto text-xs bg-primary text-white rounded-tenant-full font-bold shadow-tenant-sm transition-all">
                            {{ $cell['day'] }}
                        </a>
                    @elseif ($cell['isToday'])
                        <a href="{{ $linkUrl }}" class="ajax-link w-8 h-8 flex items-center justify-center mx-auto text-xs text-primary font-bold border border-primary rounded-tenant-full hover:bg-primary-light transition-all">
                            {{ $cell['day'] }}
                        </a>
                    @else
                        <a href="{{ $linkUrl }}" class="ajax-link w-8 h-8 flex items-center justify-center mx-auto text-xs text-gray-700 font-medium hover:bg-primary-light rounded-tenant-full transition-all">
                            {{ $cell['day'] }}
                        </a>
                    @endif
                @endforeach
    ...
    <x-tenant-card variant="default">
        ...
        <form id="rescheduleForm" method="POST" action="{{ route('tenant.booking.confirmation.reschedule') }}">
            ...
            <div class="flex justify-end gap-4 mt-6">
                ...
                <x-tenant-button type="button" id="btnConfirm" disabled variant="primary" class="px-6 py-2.5 shadow-tenant-sm">
                    Lanjut Konfirmasi
                </x-tenant-button>
            </div>
        </form>
...
<div id="reasonModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 backdrop-blur-xs transition-all duration-300">
    <div class="bg-white rounded-tenant-lg p-6 max-w-md w-full mx-4 shadow-tenant-md border border-gray-100">
        ...
        <textarea id="modalReason" rows="4" class="w-full bg-gray-50 border border-gray-200 rounded-tenant-md px-4 py-3 text-sm focus:ring-1 focus:ring-primary focus:border-primary focus:bg-white outline-none transition-all resize-none mb-6 placeholder-gray-400" placeholder="Contoh: Ada urusan mendadak..."></textarea>

        <div class="flex gap-4 justify-end">
            <x-tenant-button type="button" id="modalCancel" class="bg-gray-100 text-gray-700 hover:bg-gray-200 px-5 py-2.5">
                Batal
            </x-tenant-button>
