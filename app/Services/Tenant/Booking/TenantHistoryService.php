...
class TenantHistoryService
{
    ...
    public function getBookingDetail(int $userId, int $bookingId): Booking
    {
        ...
        $booking->details->transform(function ($detail) {
            ...
            $detail->rescheduleFee = $detailPayments->where(self::COL_PAYMENT_TYPE, PaymentType::RESCHEDULE_FEE->value)->sum('amount');
            $detail->rescheduleStatus = $detailPayments->where(self::COL_PAYMENT_TYPE, PaymentType::RESCHEDULE_FEE->value)->first()->status ?? null;
            
            $detail->refundAmount = $detailPayments->where(self::COL_PAYMENT_TYPE, PaymentType::REFUND->value)->sum('amount');
            $detail->refundStatus = $detailPayments->where(self::COL_PAYMENT_TYPE, PaymentType::REFUND->value)->first()->status ?? null;
